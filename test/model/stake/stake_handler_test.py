...
class TestStakeHandlerImpl(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.TEST_STAKE = Stake([1], Combination.HIGH_CARD)
        cls.TEST_NEXT_STAKE = Stake([2], Combination.HIGH_CARD)
        cls.TEST_TWO_PAIR = Stake([2, 5], Combination.TWO_PAIR)
        cls.TEST_NEXT_TWO_PAIR = Stake([5, 3], Combination.TWO_PAIR)
    ...
    def test_get_lowest_next_stake(self):
        self.stakeHanlder.stake = self.TEST_STAKE
        self.assertEqual(self.stakeHanlder.get_lowest_next_stake(), self.TEST_NEXT_STAKE)
        
    def test_get_lowest_next_stake_two_pair(self):
        self.stakeHanlder.stake = self.TEST_TWO_PAIR
        self.assertEqual(self.stakeHanlder.get_lowest_next_stake(), self.TEST_NEXT_TWO_PAIR)
        
    def test_get_lowest_next_stake_royal_flush(self):
        self.stakeHanlder.stake = self.TEST_ROYAL_FLUSH
        self.assertEqual(self.stakeHanlder.get_lowest_next_stake(), None)
